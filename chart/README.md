...
```sh
helm install readout oci://ghcr.io/kbelokon/charts/readout --version 0.11.5
```
...
helm upgrade --install readout oci://ghcr.io/kbelokon/charts/readout \
  --version 0.11.5 -f my-values.yaml
```
...
helm install readout oci://ghcr.io/kbelokon/charts/readout --version 0.11.5 -f my-values.yaml
```
...
| `config.live` | app defaults `{maxConnections: 512, maxSources: 128, maxCacheAccountedBytes: 128Mi}` (commented out in `values.yaml`) | Per-pod bounds on the Live (SSE) surface: open Live streams, distinct upstream LIST+watch sources, and accounted retained object bytes. Browsers on the same scope and credentials share one source, so `maxSources` sits far below `maxConnections`. Each pod enforces its own copy, so with `replicaCount > 1` the deployment-wide ceiling is that many times each value; a refused subscriber gets `429` with `Retry-After` and the page keeps working through Refresh. Lower `maxCacheAccountedBytes` when `resources.limits.memory` is below the chart default. |
...
- **Chart values** — render the manifests and let the gates run:
  ...
  ```sh
  helm template readout oci://ghcr.io/kbelokon/charts/readout --version 0.11.5 -f my-values.yaml
  ```
