...
# (decision codes like D4, plan units, backlog ids, SPEC sections, design
# docs) instead of explaining the invariant in plain words. A comment must
...
markers='\bD[0-9]+[a-z]?\b|\b(Unit|U) ?[0-9]+\b|\bB-[0-9]{3}\b|SPEC|design\.md|docs/forge|design_handoff'
...
if [ -n "${violations}" ]; then
  printf '%s\n' "${violations}"
  echo 'comment-check: design-doc references found in comments (D-codes, SPEC, docs/forge); explain the invariant in plain words instead' >&2
  exit 1
fi
exit 0
